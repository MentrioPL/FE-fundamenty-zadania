...
function simulateDataLoad(callback: () => void) {
  setTimeout(() => {
    callback()
  }, 2000)
}
...
const loadInfoBtn = document.querySelector("#loadInfoBtn") as HTMLButtonElement
const companyInfo = document.querySelector("#companyInfo") as HTMLElement
...
const ctaButton = document.querySelector("#ctaBtn") as HTMLButtonElement
const trackingOutput = document.querySelector("#tracking-output") as HTMLElement
...
/* ─────────────────────
   ZADANIE 3 – async/await, filtr, sort, render + obsługa błędów
   ───────────────────── */
...
const loadReviewsBtn = document.querySelector(
  "#load-reviews-btn"
) as HTMLButtonElement
const reviewsList = document.querySelector("#reviews-list") as HTMLElement
